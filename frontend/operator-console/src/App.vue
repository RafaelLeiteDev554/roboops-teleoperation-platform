...
type RobotStatus = 'Online' | 'Teleoperated' | 'Paused' | 'Offline'
type IngestionStatus = 'Captured' | 'Processing' | 'Validated' | 'Rejected' | 'Ingested'
type RobotCommandType = 'Move' | 'Stop' | 'Pause' | 'Resume'
...
const apiBaseUrl = import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:5157'
...
async function login() {
  const response = await request<{ accessToken: string; role: string }>('/api/auth/login', {
    method: 'POST',
    body: JSON.stringify({ username: 'admin', password: 'roboops' }),
  })
